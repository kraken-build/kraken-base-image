...
RUN : \
    && apt-get update \
    && apt-get install -y curl git wget libssl-dev libffi-dev llvm clang gcc g++ pkg-config build-essential jq sudo cloud-utils qemu-img qemu-kvm qemu-system-x86-64 qemu-system-aarch64 \
